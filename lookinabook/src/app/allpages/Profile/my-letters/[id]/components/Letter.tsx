...
const {accesstoken} = useToken()
  ...
  //const canReply = letter.replies.length === 0;
   // Можно ли ответить?
  const canReply = localReplies.length === 0;
  ...
  return (
   <div className={styles["letter-inwardly-container"]}>
      ...
      {canReply ? (
          <ReplyToLetterForm replyToId={messageId}
          onReplySent={(newReply) => setLocalReplies([newReply])}
          refetchLetter={refetch} />
      ) : (
        <p style={{ marginTop: "1rem", fontStyle: "italic" }}>You’ve already replied to this letter.</p>
      )}
 </div>
  );
};
